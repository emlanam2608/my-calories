...
].map(([id, equipment, category]) =>
  exerciseCatalogEntrySchema.parse({
    id,
    name: { en: id, vi: `${id}-vi` },
    category,
    requiredCapabilities: [equipment.replace(' ', '_')],
    environments: ['home'],
...
describe('createStarterWorkoutPlan', () => {
  it('creates three conservative sessions from the approved starter catalog', () => {
    ...
    expect(plan.sessions.flatMap((session) => session.exerciseIds)).toContain(
      'sit-to-stand',
    );
    expect(plan.sessions.every((session) => session.warmup.en.length > 0)).toBe(
      true,
    );
    expect(
      plan.sessions.every((session) => session.cooldown.vi.length > 0),
    ).toBe(true);
    expect(
      plan.sessions.flatMap((session) => session.prescriptions),
    ).toContainEqual(
  ...
  it('does not generate a plan if a required catalog movement is unavailable', () => {
    expect(() =>
      createStarterWorkoutPlan(catalog.slice(1), '2026-08-31'),
  ...
  it('excludes strength work when a clinician restriction is recorded', () => {
    ...
    expect(
      plan.sessions.flatMap((session) => session.exerciseIds),
    ).not.toContain('sit-to-stand');
    expect(plan.sessions.flatMap((session) => session.exerciseIds)).toContain(
      'treadmill-walk',
    );
  });
  ...
  it('does not use an exercise whose equipment was not selected', () => {
    expect(() =>
      createStarterWorkoutPlan(catalog, '2026-08-31', {
  ...
  it('excludes exercises tagged for a reported injury concern', () => {
    const balanceCatalog: ExerciseCatalogEntry[] = catalog.map((exercise) =>
      exercise.id === 'treadmill-walk'
        ? {
            ...exercise,
            contraindicationTags: [
              'balance_risk',
            ] satisfies ExerciseCatalogEntry['contraindicationTags'],
          }
        : exercise,
    );
